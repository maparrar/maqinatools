...
class DaoPay extends Dao{
    ...
    function create($pay){
        ...
        if(!$this->exist($pay->getId())){    
            $stmt = $this->handler->prepare("INSERT INTO Pay 
            ...
            if($stmt->execute()){
                $pay->setId(intval($this->handler->lastInsertID()));
                $created=$pay;
            }else{
     ...
     * @param int Pay identificator
     * @return Pay Pay loaded
     */
    function read($id){
        $response=null;
        if($this->exist($id)){
            $stmt = $this->handler->prepare("SELECT * FROM Pay WHERE id= ?");
    ...
    function update($pay){
        $updated=false;
        if($this->exist($pay->getId())){
            $stmt = $this->handler->prepare("UPDATE Pay SET 
                `date`=:date,
                `amount`=:amount,
                `from`=:from,
                `to`=:to,
    ...
    function delete($pay){
        $deleted=false;
        if($this->exist($pay->getId())){
            $stmt = $this->handler->prepare("DELETE Pay WHERE id=:id");
            $stmt->bindParam(':id',$pay->getId());
            if($stmt->execute()){
                $deleted=true;
            }else{
    ...
    function exist($id){
        $exist=false;
        $stmt = $this->handler->prepare("SELECT id FROM Pay WHERE id=:id");
    ...
    function listing(){
        $list=array();
        $stmt = $this->handler->prepare("SELECT id FROM Pay");
        if ($stmt->execute()) {
