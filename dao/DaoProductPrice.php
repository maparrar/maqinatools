 ...
 * @subpackage dal
 */
class DaoProductPrice{
    /**
     * Create an ProductPrice in the database
     * @param ProductPrice $productprice new ProductPrice
    ...
    function create($productprice){
        ...
        if(!$this->exist($productprice->getId())){    
            ...
            $stmt = $handler->prepare("INSERT INTO ProductPrice 
            ...
            if($stmt->execute()){
                $productprice->setId(intval($handler->lastInsertID()));
     ...
     * @param int $id ProductPrice identificator
    ...
    function read($id){
        ...
        if($this->exist($id)){
            ...
            $stmt = $handler->prepare("SELECT * FROM ProductPrice WHERE id= ?");
     ...
     * @param ProductPrice $productprice ProductPrice object
    ...
    function update($productprice){
        ...
        if($this->exist($productprice->getId())){
            ...
            $stmt = $handler->prepare("UPDATE ProductPrice SET 
                `product`=:product,
                `date`=:date,
    ...
    function delete($productprice){
        ...
        if($this->exist($productprice->getId())){
            ...
            $stmt = $handler->prepare("DELETE ProductPrice WHERE id=:id");
            $stmt->bindParam(':id',$productprice->getId());
            if($stmt->execute()){
                $deleted=true;
            }else{
    ...
    function exist($id){
        $exist=false;
        $handler=Maqinato::connect("read");
        $stmt = $handler->prepare("SELECT id FROM ProductPrice WHERE id=:id");
    ...
    function listing(){
        $list=array();
        $handler=Maqinato::connect("read");
        $stmt = $handler->prepare("SELECT id FROM ProductPrice");
        if ($stmt->execute()) {
