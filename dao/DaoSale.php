...
class DaoSale extends Dao{
    ...
    function create($sale){
        ...
        if(!$this->exist($sale->getId())){    
            $stmt = $this->handler->prepare("INSERT INTO Sale 
            ...
            if($stmt->execute()){
                $sale->setId(intval($this->handler->lastInsertID()));
                $created=$sale;
            }else{
     ...
     * @param int Sale identificator
     * @return Sale Sale loaded
     */
    function read($id){
        $response=null;
        if($this->exist($id)){
            $stmt = $this->handler->prepare("SELECT * FROM Sale WHERE id= ?");
    ...
    function update($sale){
        $updated=false;
        if($this->exist($sale->getId())){
            $stmt = $this->handler->prepare("UPDATE Sale SET 
                `date`=:date,
                `totalReal`=:totalReal,
                `paid`=:paid,
                `client`=:client,
    ...
    function delete($sale){
        $deleted=false;
        if($this->exist($sale->getId())){
            $stmt = $this->handler->prepare("DELETE Sale WHERE id=:id");
            $stmt->bindParam(':id',$sale->getId());
            if($stmt->execute()){
                $deleted=true;
            }else{
    ...
    function exist($id){
        $exist=false;
        $stmt = $this->handler->prepare("SELECT id FROM Sale WHERE id=:id");
    ...
    function listing(){
        $list=array();
        $stmt = $this->handler->prepare("SELECT id FROM Sale");
        if ($stmt->execute()) {
