...
class DaoLoan{
    ...
    function create($loan){
        ...
        if(!$this->exist($loan->getId())){    
            ...
            $stmt = $handler->prepare("INSERT INTO Loan 
            ...
            if($stmt->execute()){
                $loan->setId(intval($handler->lastInsertID()));
                $created=$loan;
            }else{
     ...
     * @param int $id Loan identificator
    ...
    function read($id){
        ...
        if($this->exist($id)){
            ...
            $stmt = $handler->prepare("SELECT * FROM Loan WHERE id= ?");
    ...
    function update($loan){
        ...
        if($this->exist($loan->getId())){
            ...
            $stmt = $handler->prepare("UPDATE Loan SET 
                `date`=:date,
                `interest`=:interest,
                `paid`=:paid,
                `from`=:from,
    ...
    function delete($loan){
        ...
        if($this->exist($loan->getId())){
            ...
            $stmt = $handler->prepare("DELETE Loan WHERE id=:id");
            $stmt->bindParam(':id',$loan->getId());
            if($stmt->execute()){
                $deleted=true;
            }else{
    ...
    function exist($id){
        $exist=false;
        $handler=Maqinato::connect("read");
        $stmt = $handler->prepare("SELECT id FROM Loan WHERE id=:id");
    ...
    function listing(){
        $list=array();
        $handler=Maqinato::connect("read");
        $stmt = $handler->prepare("SELECT id FROM Loan");
        if ($stmt->execute()) {
